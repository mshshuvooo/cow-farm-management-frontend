...
import { editVaccine } from "/src/services/vaccine/vaccine.service";
...
const errors = ref([]);
const emit = defineEmits(["update:vaccineAfterEdit"]);
...
const vaccineInfo = ref({
  vaccination_date: props.vaccine?.vaccination_date ?? "",
  dose: props.vaccine?.dose ?? "",
  next_vaccination_date: props.vaccine?.next_vaccination_date ?? "",
  vaccine_type: props.vaccine?.vaccine_type ?? "",
  cows: props.vaccine.cows?.map((cow) => cow.id) ?? [],
});

const handleEditVaccine = async (e) => {
  e.preventDefault();
  const result = await editVaccine(props.vaccine?.id, vaccineInfo);
  if (result?.data?.success) {
    emit("update:vaccineAfterEdit", result);
