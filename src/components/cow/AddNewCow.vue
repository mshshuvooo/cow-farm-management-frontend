...
import { addNewCow, getCows } from "/src/services/cow/cow.service";
import ErrorMessages from "/src/components/shared/ErrorMessages.vue";

const hasError = ref(false);
const errors = ref([]);
const emit = defineEmits(["update:cowsAfterAddNew"]);
...
const handleAddNewCow = async (e) => {
  e.preventDefault();
  const result = await addNewCow(cowInfo);
  if (result?.data?.success) {
    emit("update:cowsAfterAddNew", result);
